...
Status: validado em desenvolvimento; migrations de produção aplicadas, aguardando backfill e transição
Data: 23/07/2026
...
4. Executar backfill das chaves atuais para objetos versionados. Dispensado no desenvolvimento em 24/07/2026: a auditoria confirmou zero chaves legadas, snapshots e objetos no bucket. O inventário de produção confirmou quatro blobs legados, 974425 bytes, zero snapshots e zero objetos; as migrations de manutenção foram aplicadas em 27/07/2026 e o runner seguro aguarda o plano autenticado.
...
O carregamento passa a envolver metadados e um objeto do Storage, mas evita blobs grandes em uma tabela de configuração e preserva o modelo de snapshot já usado pela aplicação. A migration, a leitura, a integridade, as permissões, o rollback e o reset transacional foram validados em desenvolvimento. O inventário próprio de produção está registrado em `supabase_production_inventory_2026-07-24.md`; as migrations de manutenção foram confirmadas em 27/07/2026 e a transição continua condicionada ao backfill e à validação autenticada.
