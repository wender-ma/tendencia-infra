...
| EXT-02 | Bloqueada | Administrador da Vercel | Acesso ao projeto na Vercel | Conferir as quatro variáveis de Production descritas em `docs/operations.md` e executar Redeploy da revisão atual. | Deployment `Ready`, domínio apontando para a revisão atual e preflight de produção aprovado. |
| EXT-03 | Bloqueada | Responsável funcional | EXT-02 | Executar o checklist de produção em `docs/manual_validation.md`. | Data, navegador, perfil e resultado registrados no checklist. |
...
| EXT-06 | Aberta | Pessoa com leitor de tela | EXT-02 | Validar login, navegação, formulários, modais e mensagens com NVDA ou VoiceOver. | Leitor, navegador, data e resultado registrados em `docs/manual_validation.md`. |
| EXT-07 | Bloqueada | Responsável técnico | EXT-02 e janela de estabilidade | Manter produção em `snapshots`, observar a janela definida e autorizar a remoção dos quatro blobs legados. | Inventário sem regressão, janela cumprida, autorização registrada e limpeza verificada. |
...
1. Concluir EXT-02 para publicar o código já validado.
2. Executar EXT-03 e EXT-06 no domínio atualizado.
3. Iniciar a janela de EXT-07 e só então avaliar a limpeza legada.
4. Resolver EXT-01, EXT-04, EXT-05, EXT-08 e EXT-09 com os responsáveis indicados.
