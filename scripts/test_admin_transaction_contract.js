...
const deploymentAudit = fs.readFileSync(
  path.join(root, 'supabase', 'audit', 'verify_admin_transactions_deployment.sql'),
  'utf8',
);
...
assert(
  deleteObra.includes("SUPA.rpc('admin_delete_obra'"),
  'Exclusão de obra precisa usar a RPC transacional',
);
[
...
assert(
  deleteObra.includes('cleanupDeletedObraStorage('),
  'Arquivos da obra precisam ser limpos após o commit',
);

const saveUser = extract(
  javascript,
  'async function salvarEditorForm(',
  '// Excluir usuário direto do modal',
);
assert(
  saveUser.includes("SUPA.rpc('admin_replace_user_permissions'"),
  'Permissões precisam ser substituídas via RPC',
);
assert(
  !saveUser.includes("from('editores_permitidos').delete()"),
  'Permissões não podem ser apagadas antes do novo escopo',
);
assert(
  !saveUser.includes("from('editores_permitidos').insert("),
  'Permissões não podem ser inseridas em chamada separada',
);

const deleteUser = extract(
  javascript,
  'async function excluirUsuarioDoModal(',
  '// -------------- PENDENTES',
);
assert(
  deleteUser.includes("SUPA.rpc('admin_delete_user_permissions'"),
  'Exclusão de usuário precisa usar a RPC protegida',
);
...
[
  ...
  assert(
    migration.toLowerCase().includes(contract.toLowerCase()),
    `Contrato SQL ausente: ${contract}`,
  ),
);
...
[
  ...
  'admin_delete_user_permissions(text)',
  'admin_delete_obra(text)',
  "pg_notify('pgrst', 'reload schema')",
  'admin_transactions_deployment',
].forEach((contract) =>
