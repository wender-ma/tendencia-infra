...
// Função unificada de parsing de números (substitui parseNum, parseNumBR, parsePct)
// Aceita: formato BR ("1.614.494,59"), US ("1,614,494.59"), porcentagens ("31,5%")
// Opções:
//   - isPct: se true, trata frações 0-1 como porcentagens (multiplica por 100)
//   - allowPlus: se true, remove '+' do início
function parseNumero(s, opts = {}) {
  if (s == null) return null;
  s = String(s).trim();
  if (!s || s === '-' || s.includes('#REF')) return null;
  
  // Limpeza básica
  s = s.replace(/R\$/g, '').replace(/\s/g, '').replace(/%/g, '');
  if (opts.allowPlus) s = s.replace(/^\+/, '');
  if (!s) return null;
  
  // Detecta formato pelo separador que aparece por último (é o decimal)
  const lastComma = s.lastIndexOf(',');
  const lastDot = s.lastIndexOf('.');
  let normalized;
  
  if (lastComma > lastDot) {
    // Formato BR: vírgula decimal, ponto milhar → "1.614.494,59" → "1614494.59"
    normalized = s.replace(/\./g, '').replace(',', '.');
  } else if (lastDot > lastComma) {
    // Formato US: ponto decimal, vírgula milhar → "1,614,494.59" → "1614494.59"
    normalized = s.replace(/,/g, '');
  } else {
    // Só um separador ou nenhum: interpreta vírgula como decimal (padrão BR)
    normalized = s.replace(',', '.');
  }
  
  // Rejeita notação científica ("1e10") e valores não-finitos ("Infinity")
  if (/[eE]/.test(normalized)) return null;
  const n = parseFloat(normalized);
  if (isNaN(n) || !isFinite(n)) return null;
  
  // Se for porcentagem e o valor for uma fração 0-1, multiplica por 100
  if (opts.isPct && n > 0 && n <= 1.5 && !s.includes('%')) {
    return n * 100;
  }
...
// parseNum/parseNumBR/parsePct removidos — use parseNumero() diretamente
...
function authToast(msg, kind='info', ms=3500) {
  ...
  el.className = kind === 'err' ? 'err' : kind === 'warn' ? 'warn' : kind === 'ok' ? 'ok' : '';
  el.textContent = msg;
...
function getModalDialog(backdrop) {
...
function openModalLayer(backdrop, options = {}) {
  if (!backdrop) return false;
  const existing = MODAL_STACK.find(entry => entry.backdrop === backdrop);
  if (existing) closeModalLayer(backdrop, false);

  const dialog = getModalDialog(backdrop);
  const previousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  if (ACTIVE_MODAL) ACTIVE_MODAL.backdrop.setAttribute('aria-hidden', 'true');
  const entry = {
    backdrop,
    dialog,
    previousFocus,
  ...
  backdrop.style.zIndex = String(2000 + MODAL_STACK.length * 10);
  document.body.classList.add('modal-open');

  requestAnimationFrame(() => {
    if (!ACTIVE_MODAL || ACTIVE_MODAL.backdrop !== backdrop) return;
    const requested = typeof options.initialFocus === 'string'
      ? backdrop.querySelector(options.initialFocus)
      : options.initialFocus;
    const target = requested || getModalFocusableElements(dialog)[0] || dialog;
    target?.focus();
  });
  return true;
}

function closeModalLayer(backdrop, result = false) {
  if (!backdrop) return;
  const index = MODAL_STACK.findIndex(entry => entry.backdrop === backdrop);
  if (index < 0) {
  ...
  const wasActive = index === MODAL_STACK.length - 1;
  const [{ previousFocus, onClose }] = MODAL_STACK.splice(index, 1);
  backdrop.classList.remove('show');
  backdrop.setAttribute('aria-hidden', 'true');
  backdrop.style.removeProperty('z-index');
  ACTIVE_MODAL = MODAL_STACK[MODAL_STACK.length - 1] || null;
  if (ACTIVE_MODAL) ACTIVE_MODAL.backdrop.setAttribute('aria-hidden', 'false');
...
function openModal(options = {}) {
  ...
  const title = document.querySelector('#modalContent h1, #modalContent h2, #modalContent h3');
  if (dialog) dialog.setAttribute('aria-label', title?.textContent?.trim() || 'Janela do dashboard');
  return openModalLayer(backdrop, options);
}

function closeModal(result = false) {
  closeModalLayer(document.getElementById('modalBg'), result);
}

function closeConfirmModal(result = false) {
  closeModalLayer(document.getElementById('confirmModalBg'), result);
}
...
document.addEventListener('keydown', event => {
  ...
  if (event.key === 'Escape') {
    event.preventDefault();
    closeModalLayer(ACTIVE_MODAL.backdrop, false);
    return;
  }
  if (event.key !== 'Tab') return;

  const focusable = getModalFocusableElements(ACTIVE_MODAL.dialog);
  if (!focusable.length) {
    event.preventDefault();
    ACTIVE_MODAL.dialog?.focus();
    return;
  }
  const first = focusable[0];
  const last = focusable[focusable.length - 1];
  if (!ACTIVE_MODAL.dialog?.contains(document.activeElement)) {
    event.preventDefault();
...
// Formulários dinâmicos usam submit para funcionar igualmente por clique e Enter.
document.getElementById('modalContent')?.addEventListener('submit', event => {
  ...
  if (action === 'save-manual') saveManualForm(submitter.dataset.n);
  else if (action === 'save-mov') saveMovForm(submitter.dataset.id || '');
  else if (action === 'mass-confirm') massConfirmCallback();
});

// Modal de confirmação customizado (substitui a confirmação nativa do navegador)
function confirmModal(titulo, mensagem, opts) {
  opts = opts || {};
  const btnConfirmText = opts.confirmText || 'Confirmar';
  const btnCancelText = opts.cancelText || 'Cancelar';
  const isDestructive = opts.destructive !== false;
  const requireText = opts.requireText || null; // texto que o usuário deve digitar para habilitar OK

  return new Promise(function(resolve) {
    const confirmBtnStyle = isDestructive
      ? 'background:var(--sem-erro); color:white; border:1px solid var(--sem-erro);'
      : 'background:var(--fgr-red-dark); color:white; border:1px solid var(--fgr-red-dark);';

    const requireHtml = requireText
      ? '<div style="margin-top:12px;">'
        + '<label for="confirmModalRequire" style="font-size:11px; text-transform:uppercase; color:var(--text-soft); font-weight:600;">Digite <code>' + escHtml(requireText) + '</code> para confirmar:</label>'
        + '<input type="text" id="confirmModalRequire" style="width:100%; padding:8px 10px; border:1px solid var(--border-strong); border-radius:6px; margin-top:4px; font-size:13px; font-family:monospace;" autocomplete="off">'
        + '</div>'
      : '';
    ...
    document.getElementById('confirmModalContent').innerHTML = ''
      ...
      + '<div style="white-space:pre-wrap; font-size:13px; line-height:1.6; color:var(--text-medium); margin:16px 0;">' + escHtml(mensagem) + '</div>'
      + requireHtml
      + '<div style="display:flex; gap:8px; justify-content:flex-end; margin-top:16px;">'
      + '  <button type="button" class="btn-sm" id="confirmModalCancel">' + escHtml(btnCancelText) + '</button>'
      + '  <button type="submit" class="btn-sm" id="confirmModalOk"' + (requireText ? ' disabled' : '') + ' style="' + confirmBtnStyle + (requireText ? '; opacity:0.5; cursor:not-allowed;' : '') + '">' + escHtml(btnConfirmText) + '</button>'
      + '</div>'
      + '</form>';
    ...
    if (requireText) {
      reqInput = document.getElementById('confirmModalRequire');
    ...
    form.addEventListener('submit', event => {
      event.preventDefault();
      if (reqInput && reqInput.value.trim() !== requireText) return;
      closeConfirmModal(true);
    });
    cancelBtn.onclick = function() { closeConfirmModal(false); };
    openModalLayer(document.getElementById('confirmModalBg'), {
      onClose: resolve,
      initialFocus: requireText ? '#confirmModalRequire' : '#confirmModalCancel',
    });
  });
}
...
const GRUPOS_MAP = {
  '01.01': 'Custos Indiretos', '01.02': 'Custos Diretos / Infraestrutura',
  '01.03': 'Obras Civis', '01.04': 'Projeção de Gastos',
  '01.09': 'Serviços Iniciais', '09.01': 'Serviços Iniciais Adicionais',
  '09.02': 'Serviços Iniciais Adicionais',
};

const IMPORT_HEADER_RULES = {
  tendencia: {
    label: 'TENDÊNCIA',
    positional: [
      { index: 1, label: 'Código', alternatives: [['codigo'], ['cod']] },
      { index: 2, label: 'Serviço', alternatives: [['servico']] },
      { index: 3, label: 'Insumo', alternatives: [['insumo']] },
      { index: 4, label: 'Item', alternatives: [['item']] },
      { index: 6, label: 'Orçamento Licitação', alternatives: [['licitacao']] },
      { index: 7, label: 'IPCA', alternatives: [['ipca']] },
      { index: 8, label: 'INCC', alternatives: [['incc']] },
      { index: 9, label: 'Gestão', alternatives: [['gestao']] },
      { index: 10, label: 'Diferença', alternatives: [['diferenca']] },
      { index: 12, label: 'Evolução Teórica', alternatives: [['evolucao', 'teorica']] },
      { index: 13, label: 'Evolução Financeira', alternatives: [['evolucao', 'financeira']] },
    ],
  },
  flows: {
    label: 'FLOWS',
    positional: [
      { index: 0, label: 'Cod_aditivo', alternatives: [['cod', 'aditivo']] },
      { index: 1, label: 'Descr_status', alternatives: [['descr', 'status']] },
      { index: 2, label: 'Descr_areaatual', alternatives: [['descr', 'areaatual']] },
      { index: 3, label: 'Descr_setorcriacao', alternatives: [['descr', 'setorcriacao']] },
      { index: 4, label: 'Data_criacao', alternatives: [['data', 'criacao']] },
      { index: 5, label: 'Descr_motivo', alternatives: [['descr', 'motivo']] },
      { index: 6, label: 'Descr_observacao_motivo', alternatives: [['descr', 'observacao', 'motivo']] },
      { index: 7, label: 'Descr_descricaoaditivo', alternatives: [['descr', 'descricaoaditivo']] },
      { index: 8, label: 'Cod_obra', alternatives: [['cod', 'obra']] },
      { index: 9, label: 'Valor Aprovado ou Solicitado', alternatives: [['valor', 'aprovado'], ['valor', 'solicitado']] },
      { index: 10, label: 'Vlr_planejamento', alternatives: [['vlr', 'planejamento'], ['valor', 'planejamento']] },
      { index: 11, label: 'Departamento', alternatives: [['departamento']] },
      { index: 12, label: 'Ins. Planej.', alternatives: [['ins', 'planej'], ['insumo', 'planejamento']] },
      { index: 13, label: 'Ins. Remanej.', alternatives: [['ins', 'remanej'], ['insumo', 'remanejamento']] },
      { index: 14, label: 'Refletido', alternatives: [['refletido']] },
    ],
  },
  gestoes: {
    label: 'GESTÕES',
    named: [
      { label: 'Descr_gestao', alternatives: [['descr', 'gestao']] },
      { label: 'Descr_classificacaofinanceira', alternatives: [['descr', 'classificacaofinanceira']] },
      { label: 'Key_planejamento', alternatives: [['key', 'planejamento']] },
      { label: 'Val_totalliquido', alternatives: [['val', 'totalliquido'], ['valor', 'total', 'liquido']] },
      { label: 'Mes_pagamento', alternatives: [['mes', 'pagamento']] },
    ],
  },
};
...
function importHeaderMatches(value, alternatives) {
  const normalized = normalizeImportHeader(value);
  return alternatives.some(terms => terms.every(term => normalized.includes(term)));
}

function validateImportHeaders(kind, rows) {
  ...
  (rules.positional || []).forEach(rule => {
    if (importHeaderMatches(headers[rule.index], rule.alternatives)) return;
    const foundIndex = headers.findIndex(value => importHeaderMatches(value, rule.alternatives));
    if (foundIndex >= 0) {
      misplaced.push(`${rule.label} (esperado na coluna ${rule.index + 1}, encontrado na ${foundIndex + 1})`);
    } else {
      missing.push(rule.label);
    }
  });

  (rules.named || []).forEach(rule => {
    if (!headers.some(value => importHeaderMatches(value, rule.alternatives))) missing.push(rule.label);
  });

  if (missing.length || misplaced.length) {
    const details = [];
    if (missing.length) details.push(`ausentes: ${missing.join(', ')}`);
    if (misplaced.length) details.push(`posição incorreta: ${misplaced.join('; ')}`);
    throw new Error(
      `${rules.label}: cabeçalho inválido (${details.join(' | ')}). ` +
      'Nenhum dado foi importado. Confirme o layout e exporte o CSV UTF-8 separado por ponto e vírgula.'
    );
  }
...
// parseNum agora é um alias para parseNumero (definido no início do script)

// v0.55 — Formato novo da aba TENDÊNCIA (22 colunas)
// Adiciona colunas EVOLUÇÃO TEÓRICA e EVOLUÇÃO FINANCEIRA (do Prevision)
// Layout:
// [0] chave · [1] cod · [2] cod_servico · [3] cod_insumo · [4] item
// [5] (vazio separador) · [6] ORÇ. LICITAÇÃO · [7] IPCA · [8] INCC · [9] GESTÃO · [10] DIFERENÇA
// [11] (vazio) · [12] EVOL. TEÓRICA % · [13] EVOL. FINANCEIRA % · [14] (vazio)
// [15] TENDÊNCIA · [16] DIF. TENDÊNCIA · [17] (vazio) · [18] OBSERVAÇÕES · [19+] vazios
// Linha 0 = cabeçalho, linha 1 = subheader com totais globais (área vendável + % globais)
function parseTendencia(text) {
  // Usa parseCSVRows para lidar com aspas multilinha (headers têm \n dentro de "")
  const rows = parseCSVRows(text);
  validateImportHeaders('tendencia', rows);
  if (rows.length < 3) throw new Error('TENDÊNCIA: arquivo sem linhas de dados.');

  // Capturar label da gestão (agora está no idx 9)
  try {
    const headerCol9 = (rows[0][9] || '').trim().replace(/\s+/g, ' ');
    if (headerCol9 && headerCol9.length > 3) {
      GESTAO_LABEL = headerCol9;
    }
  } catch(e) { reportNonFatalError('Tendência/ler cabeçalho de gestão', e); }

  // Capturar totais globais da linha 1 (subheader)
  // [12] = EVOL. TEÓRICA global (%) · [13] = EVOL. FINANCEIRA global (%)
  // ⚠️ Excel/SheetJS pode entregar como "31,00%" (BR), "31.00%" (EN),
  //    "31" (número), ou "0.31" (fração 0-1 quando rawNumbers=true).
  //    Precisamos ser robustos com todos.
  EVOL_GLOBAL = { teorica: null, financeira: null };
  try {
    const sub = rows[1] || [];
    const parsePct = (raw) => {
      ...
      // Limpa tudo: R$, espaço, %, pontos/vírgulas
      s = s.replace(/[R\$\s%]/g, '');
      if (!s) return null;
      // Detectar formato: se tem vírgula E ponto, o último decide
      const lastComma = s.lastIndexOf(',');
      const lastDot   = s.lastIndexOf('.');
      let normalized;
      if (lastComma > lastDot) {
        // Vírgula é decimal (BR): '1.234,56' → '1234.56'
        normalized = s.replace(/\./g, '').replace(',', '.');
      } else if (lastDot > lastComma) {
        // Ponto é decimal (EN): '1,234.56' → '1234.56'
        normalized = s.replace(/,/g, '');
      } else {
        // Só um separador ou nenhum
        normalized = s.replace(',', '.');
      }
      const n = parseFloat(normalized);
      if (isNaN(n)) return null;
      // Se veio como fração 0-1 (ex: 0.31) e o texto original NÃO tinha %,
      // provavelmente é fração — multiplica por 100.
      // Se veio como 0.31 COM % explícito, mantém (raro mas possível: 0,31%).
      // Regra prática: se número < 1.5 e sem % explícito, assume fração.
      // Se número < 1.5 e COM %, mantém (é um % genuíno pequeno).
      if (!hadPercent && n > 0 && n <= 1.5) return n * 100;
      return n;
    };
    EVOL_GLOBAL.teorica    = parsePct(sub[12]);
    EVOL_GLOBAL.financeira = parsePct(sub[13]);
    console.log('[TEND] EVOL_GLOBAL parsed:', EVOL_GLOBAL,
                '(bruto: [12]="' + sub[12] + '", [13]="' + sub[13] + '")');
    // v0.55.2 + v0.58a: persistir no Supabase (chave prefixada com codigo_obra)
    try {
      if (isEditorDaObraAtiva() && OBRA_ATIVA) {
        localStorage.setItem('jzurique_evol_global', JSON.stringify(EVOL_GLOBAL));
        if (typeof supaSaveDashboardKey === 'function') {
          supaSaveDashboardKey(keyPorObra('evol_global'), JSON.stringify(EVOL_GLOBAL));
        }
      }
    } catch(e) { console.warn('[TEND] persist EVOL err:', e); }
  } catch(e) { console.warn('[TEND] EVOL_GLOBAL parse err:', e); }

  const out = [];
  // Dados a partir da linha 2 (linha 1 é subheader com totais)
  for (let i = 2; i < rows.length; i++) {
    const cols = rows[i];
    if (!cols || cols.length < 11) continue;
    const cod = (cols[1]||'').trim();
    const cod_servico_raw = (cols[2]||'').trim();
    const cod_servico = (cod_servico_raw === '-1') ? '' : cod_servico_raw;
    const cod_insumo_raw = (cols[3]||'').trim();
    // 'INSUMOS' e '-1' são marcadores, não insumos reais
    const cod_insumo = (cod_insumo_raw === '-1' || cod_insumo_raw === 'INSUMOS') ? '' : cod_insumo_raw;
    const item = (cols[4]||'').trim();
    if (!cod || !item) continue;
    const parts = cod.split('.');
    const grupo_cod = parts.length >= 2 ? parts.slice(0,2).join('.') : cod;
    const grupo = GRUPOS_MAP[grupo_cod] || grupo_cod;
    const is_folha = Boolean(cod_insumo);
    const licitacao = parseNum(cols[6]);
    const ipca = parseNum(cols[7]);
    const incc = parseNum(cols[8]);
    const gestao = parseNum(cols[9]);
    const diferenca = parseNum(cols[10]);
    // Evolução: pode vir como número (%) ou como valor absoluto no total geral.
    // Nos itens folha vem como valor R$ (não %). Tratamos como número simples.
    const evol_teor = parseNum(cols[12]);
    const evol_fin  = parseNum(cols[13]);
    out.push({
      cod, cod_servico, cod_insumo, item, grupo_cod, grupo,
      nivel: parts.length, is_folha,
      licitacao,
      corrigido_incc: incc,
      corrigido_ipca: ipca,
      licitacao_corrigido: (CORRECAO_INDICE === 'ipca') ? ipca : incc,
      gestao,
      diferenca,
      // colunas novas
      evolucao_teorica:    evol_teor,
      evolucao_financeira: evol_fin,
    });
  }
  return out;
}
...
function normInsumo(v) {
  v = (v||'').trim();
  if (['', '-', 'Não encontrado!', 'Cancelado'].includes(v)) return v;
  if (v.toUpperCase().includes('VERIFICAR')) return 'VERIFICAR';
  return v;
}

function classifyFlow(ip, ir) {
  const ipNorm = normInsumo(ip);
  const irNorm = normInsumo(ir);
  // se destino OU origem estão marcados como Cancelado, o aditivo é cancelado
  if (ipNorm === 'Cancelado' || irNorm === 'Cancelado') return 'cancelado';
  const isReal = v => v && !['', '-', 'Não encontrado!', 'VERIFICAR', 'Cancelado'].includes(v);
  if (ipNorm === 'Não encontrado!' || irNorm === 'VERIFICAR') return 'pendente';
  if (isReal(ipNorm) && isReal(irNorm)) return 'remanejamento';
  if (isReal(ipNorm) && irNorm === '-') return 'aumento_real';
  if (ipNorm === '-' && isReal(irNorm)) return 'economia';
  if (!isReal(ipNorm) && !isReal(irNorm)) return 'sem_classificacao';
  return 'misto';
}
...
function toIsoDate(value, preferredOrder = 'br') {
...
// v0.58b: parseGestoes agora processa TODAS as obras (não filtra por empreendimento).
// Cada item ganha campo `codigo_obra` derivado do prefixo da Key_planejamento.
// Ex: "42-21O-1-31005-..." → codigo_obra = "42-21O"
// A filtragem por obra ativa acontece na renderização (renderHistorico / renderProjecao).
function parseGestoes(text) {
  const rows = parseCSVRows(text);
  validateImportHeaders('gestoes', rows);
  if (rows.length < 2) throw new Error('GESTÕES: arquivo sem linhas de dados.');
  const header = rows[0].map(c => (c||'').trim());
  const requiredIdx = (label) => {
    const rule = IMPORT_HEADER_RULES.gestoes.named.find(item => item.label === label);
    return header.findIndex(value => importHeaderMatches(value, rule.alternatives));
  };
  const optionalIdx = (name) => {
    const normalizedName = normalizeImportHeader(name);
    return header.findIndex(value => normalizeImportHeader(value) === normalizedName);
  };

  const iGest = requiredIdx('Descr_gestao');
  const iClasFin = requiredIdx('Descr_classificacaofinanceira');
  const iKeyPlan = requiredIdx('Key_planejamento');
  const iVal = requiredIdx('Val_totalliquido');
  const iMesPag = requiredIdx('Mes_pagamento');
  const iInsumo = optionalIdx('Insumo');
  const iItem = optionalIdx('Item');
  const iServ = optionalIdx('Serviço');

  // Extrai codigo_obra do prefixo da Key_planejamento
  // "42-21O-1-31005-..." → "42-21O"
  function extrairObra(keyPlan) {
    if (!keyPlan) return null;
    const partes = keyPlan.split('-');
    if (partes.length < 2) return null;
    return partes[0] + '-' + partes[1];
  }

  const agg = new Map(); // (codigo_obra + '|' + key) → { meta, gestoes: {nome:soma} }
  const allGestoesSet = new Set();

  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r || r.length < 5) continue;
    const classFin = (r[iClasFin]||'').trim();
    const key = (r[iKeyPlan]||'').trim();
    const gestao = (r[iGest]||'').trim();
    // Filtros mínimos (não filtra por empreendimento — aceita todas as obras)
    if (classFin !== 'Obra') continue;
    if (!key) continue;
    if (!gestao) continue;
    const codigoObra = extrairObra(key);
    if (!codigoObra) continue;
    allGestoesSet.add(gestao);
    const aggKey = codigoObra + '|' + key;
    if (!agg.has(aggKey)) {
      // v0.60.2 FIX: se Serviço/Insumo/Item vieram vazios no CSV, extrair da própria Key_planejamento.
      // Formato da key: "PP-OO-N-XXXXX-SERVICO-INSUMO-ITEM"
      // Ex: "42-21O-1-31005-S05765-ADM5189-01.01.01.02"
      let servico = (r[iServ]||'').trim();
      let insumo = (r[iInsumo]||'').trim();
      let itemCod = (r[iItem]||'').trim();
      if (!servico || !insumo || !itemCod) {
        const partes = key.split('-');
        // partes[0]=PP, [1]=OO, [2]=N, [3]=XXXXX, [4]=SERVICO, [5]=INSUMO, [6+]=ITEM
        if (partes.length >= 7) {
          if (!servico) servico = partes[4];
          if (!insumo) insumo = partes[5];
          if (!itemCod) itemCod = partes.slice(6).join('-');
        }
      }
      agg.set(aggKey, {
        meta: {
    ...
    entry.gestoes[gestao] = (entry.gestoes[gestao]||0) + (parseNum(r[iVal])||0);
  }

  // Ordem cronológica das gestões
  function sortKey(g) {
    if (g === 'Atual') return [9999, 99];
    const m = g.match(/GEST[ÃA]O\s+(\d{2})-(\d{4})/i);
    if (m) return [parseInt(m[2]), parseInt(m[1])];
    return [0, 0];
  }
  const gestoes = [...allGestoesSet].sort((a,b) => {
    const sa = sortKey(a), sb = sortKey(b);
    return sa[0]*100 + sa[1] - (sb[0]*100 + sb[1]);
  });

  const items = [];
  for (const [aggKey, v] of agg) {
    const key = aggKey.substring(aggKey.indexOf('|') + 1);
    const obj = { key, ...v.meta };
    gestoes.forEach(g => { obj[g] = Math.round((v.gestoes[g]||0)*100)/100; });
    items.push(obj);
  }
  // NOTA: totals agora é POR OBRA (não somamos todas as obras juntas)
  // { "42-21O": { "GESTÃO 05-2026": xxx, ... }, "18-JROMO": { ... } }
  const totals = {};
  items.forEach(it => {
    if (!totals[it.codigo_obra]) totals[it.codigo_obra] = {};
    gestoes.forEach(g => {
      totals[it.codigo_obra][g] = (totals[it.codigo_obra][g] || 0) + (it[g] || 0);
    });
  });
  // Arredondar totals
  Object.keys(totals).forEach(obra => {
    gestoes.forEach(g => {
      totals[obra][g] = Math.round((totals[obra][g] || 0) * 100) / 100;
    });
  });

  // Construir PROJ_RAW (todas as obras, apenas gestão "Atual", mês a mês)
  PROJ_RAW = [];
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r || r.length < 5) continue;
    if ((r[iClasFin]||'').trim() !== 'Obra') continue;
    const key = (r[iKeyPlan]||'').trim();
    if (!key) continue;
    const codigoObra = extrairObra(key);
    if (!codigoObra) continue;
    if ((r[iGest]||'').trim() !== 'Atual') continue;
    const dataPagamento = toIsoDate((r[iMesPag]||'').trim(), 'br');
    if (!dataPagamento) continue;
    const yyyy_mm = dataPagamento.slice(0, 7);
    const valor = parseNum(r[iVal]) || 0;
    if (valor <= 0) continue;
    // v0.60.2 FIX: mesmo fallback pra Serviço/Insumo vazios
    let pServico = (r[iServ]||'').trim();
    let pInsumo = (r[iInsumo]||'').trim();
    if (!pServico || !pInsumo) {
      const partes = key.split('-');
      if (partes.length >= 6) {
        if (!pServico) pServico = partes[4];
        if (!pInsumo) pInsumo = partes[5];
      }
    }
    PROJ_RAW.push({
      codigo_obra: codigoObra,
  ...
  return { gestoes, items, totals };
}

// v0.63 — Parser Flows FORMATO NOVO (Fabric)
// Substitui parseFlowsValor da v0.62
//
// Formato esperado:
//   Linha 1: cabeçalho
//   Linha 2+: dados
//   12 colunas na ordem:
//     Cod_aditivo | Cod_obra | Departamento | Data | Descrição | Motivo |
//     Justificativa | Custo Flowmaster | Custo Planejamento |
//     INSUMO PLANEJAMENTO | INSUMO DE REMANEJAMENTO | REFLETIDO
//
// Cod_obra vem só como sufixo (21O, JROMO, JFRAO, ...) — mapeia pra codigo_obra
// completo via lookup na tabela OBRAS carregada do Supabase.

function parseFlowsValor(text) {
  // v0.63.1 — Parser Flows FORMATO FABRIC (ordem oficial)
  //
  // Ordem esperada das 15 colunas (linha 1 = header):
  //   0: Cod_aditivo
  //   1: Descr_status              — usado pra calcular Departamento (via fórmula Excel, mas guardamos raw também)
  //   2: Descr_areaatual           — usado pra calcular Departamento
  //   3: Descr_setorcriacao        — solicitante_dep
  //   4: Data_criacao
  //   5: Descr_motivo
  //   6: Descr_observacao_motivo   — justificativa
  //   7: Descr_descricaoaditivo    — descrição
  //   8: Cod_obra                  — só sufixo (JROMO, 21O, ...) — mapeia pro codigo_obra completo
  //   9: Valor Aprovado ou Solicitado — custo_flowmaster
  //  10: Vlr_planejamento          — custo_planejamento
  //  11: Departamento              — calculado no Excel via fórmula
  //  12: Ins. Planej.              — MANUAL (preenchido no dashboard)
  //  13: Ins. Remanej.             — MANUAL (preenchido no dashboard)
  //  14: Refletido                 — MANUAL (preenchido no dashboard)

  const rows = parseCSVRows(text);
  validateImportHeaders('flows', rows);
  if (rows.length < 2) throw new Error('FLOWS: arquivo sem linhas de dados.');

  // Mapa: sufixo da obra (Fabric) → codigo_obra completo do Supabase
  // Ex: "21O" → "42-21O", "JROMO" → "18-JROMO"
  const mapSufixoObra = {};
  if (Array.isArray(OBRAS)) {
    OBRAS.forEach(o => {
      if (!o.codigo_obra) return;
      const partes = String(o.codigo_obra).split('-');
      if (partes.length >= 2) {
        const sufixo = partes.slice(1).join('-');
        mapSufixoObra[sufixo] = o.codigo_obra;
      }
      mapSufixoObra[o.codigo_obra] = o.codigo_obra;
    });
  }

  const out = [];
  let ignoradas = 0;
  const obrasDesconhecidas = new Set();

  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r || r.length < 2) continue;

    const codAditivo = String(r[0]||'').trim();
    if (!codAditivo || !/^\d+$/.test(codAditivo)) continue;

    // Cod_obra está na posição 8
    const codObraFabric = String(r[8]||'').trim();
    const codigoObraCompleto = mapSufixoObra[codObraFabric] || null;
    if (!codigoObraCompleto) {
      obrasDesconhecidas.add(codObraFabric);
      ignoradas++;
      continue;
    }

    const descrStatus       = String(r[1]||'').trim();
    const descrAreaAtual    = String(r[2]||'').trim();
    const solicitanteDep    = String(r[3]||'').trim();
    const dataStr           = String(r[4]||'').trim();
    const dataIso           = toIsoDate(dataStr, 'br');
    const motivo            = String(r[5]||'').trim();
    const justificativa     = String(r[6]||'').trim().slice(0, 400);
    const descricao         = String(r[7]||'').trim().slice(0, 300);
    // r[8] já foi consumido (Cod_obra)
    const custoFlow         = parseNum(r[9]);
    const custoPlan         = parseNum(r[10]);
    const departamento      = String(r[11]||'').trim();
    const insumoPlanejamento = String(r[12]||'').trim();
    const insumoRemanejamento = String(r[13]||'').trim();
    const refletido         = String(r[14]||'').trim();

    // Normalizar REFLETIDO pra padrão do dashboard
    let refletidoStatus = 'pendente';
    const refLow = refletido.toLowerCase();
    if (refLow === 'sim' || refLow === 's' || refLow === 'yes' || refLow === 'refletido') refletidoStatus = 'sim';
    else if (refLow === 'não' || refLow === 'nao' || refLow === 'n' || refLow === 'no') refletidoStatus = 'nao';

    // Fallback: se Departamento veio vazio no CSV, calcula aqui pela mesma regra
    // (redundância defensiva — a fórmula do Excel já faz isso)
    let dep = departamento;
    if (!dep) {
      dep = (descrAreaAtual === 'Fora da Esteira de Aprovação') ? descrStatus : descrAreaAtual;
    }

    out.push({
      // Chave e obra
      n_alteracao: codAditivo,
      n_adt: codAditivo,
      codigo_obra: codigoObraCompleto,
      ...
      // Data
      data: dataIso,
      data_br: isoDateToBr(dataIso),

      // Descrição / motivo / justificativa
      descricao: descricao,
      motivo: motivo,
      justificativa: justificativa,

      // Valores
      custo_flowmaster: custoFlow,
      custo_planejamento: custoPlan,

      // Classificação manual (INSUMO_*)
      insumo_planejamento: insumoPlanejamento,
      insumo_remanejamento: insumoRemanejamento,
      tipo: classifyFlow(insumoPlanejamento, insumoRemanejamento),

      // Refletido
      refletido_status: refletidoStatus,
      incl_tendencia: refletido,

      // Extras do Fabric
      descr_status: descrStatus,           // valor cru (útil pra debug)
      descr_areaatual: descrAreaAtual,     // valor cru
      solicitante_dep: solicitanteDep,

      // Campos vazios do formato antigo (compat de UI)
      aprovador_dep: '',
      aprovador: '',
      solicitante: '',
      incl_orcamento: '',
  ...
  if (ignoradas > 0) {
    console.warn(`[FLOWS v0.63] ${ignoradas} linha(s) ignorada(s) — obras não cadastradas no catálogo: ${[...obrasDesconhecidas].join(', ')}`);
  }
  console.log(`[FLOWS v0.63] ${out.length} aditivo(s) importado(s).`);
